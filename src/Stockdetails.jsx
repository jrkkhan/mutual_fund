        ...
        <div className="p-6 space-y-4 ">
          ...
          <div className="text-center absolute bottom-0 left-0 flex flex-row items-center w-full px-6 bg-white py-4 space-x-4 border-t ">
            <button className=" border border-green-500 text-green-500
                    w-full p-3 rounded-lg uppercase font-medium transition duration-200   shadow-md ">sell</button>
            <button onClick={() => setOpenModal(true)} className="bg-green-500 text-white w-full p-3 rounded-lg uppercase font-medium transition duration-200 hover:bg-opacity-90 shadow-md">buy</button>
          </div>
        </div>
      </div>

      { openModal && <Stockbuy closeModal = {setOpenModal} />}
    </>
